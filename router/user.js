...
router.post('/addStudent', (req, res) => {
    UserModel.create({
        ...
        if (err) {
            ...
            doc ? resData = { code: 0, message: 'add new student successfully', userInfo: doc } : { code: 2, message: 'add new student failed', userInfo: null }
        }
        res.send(resData);
        return;
    })
...
router.post('/getStudentList', (req, res) => {
    UserModel.find(null, 'info lastTime loginCount sex username', function (err, docs) {
        if (err) {
            resData = ({ code: 1, message: 'query error' });
        } else {
            docs ? resData = { code: 0, message: 'get student list successfully', userInfo: docs } : resData = { code: 2, message: 'get student list failed', userInfo: docs };
        }
        res.send(resData);
        return;
    });
